function [ CQI_out,  SINR_rx_dB_10] = SINR_Channel_Model_Multi_Var_BS( D2D_dist, h_BS, h_MS, fc, tx_p_dBm, bandwidth, Intf_h_BS, Intf_h_MS, Intf_dist, Intf_pwr_dBm, DS_Desired, CP)
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
%  Limitations:  DS_Desired is not per BS (we assume flat interference from
%  neighboring BS, i.e. DS = 0 as they are assumed to always be transmitting;
...
%Note: CP and DS_Desired are in us (10^-6), convert to ns (10 ^ -9) representation
DS_Desired = DS_Desired * 1000;
CP = CP * 1000;
%Intf_DS_Desired = Intf_DS_Desired * 1000;
%Tapped Delay Line model requires ns representation for scaling
TDL_C_SCALED = TDL_C .* [DS_Desired * 10^(-9), 1] ; %DS_Desired (ns) is the Delay Spread as observed by the UE
%N_Intf_BS = length(Intf_DS_Desired);
%Intf_TDL_C_SCALED = zeros(N_Intf_BS,size(TDL_C_SCALED));
%for j = 1:N_Intf_BS
%    Intf_TDL_C_SCALED(j,:,:) = TDL_C .* [Intf_DS_Desired * 10^(-9), 1] ;
%end
%CP must be on same scale, for comaprison
CP = CP * 10^-9;
...
%LOS model (only for 10 meters or more)
Intf_SF = normrnd(0, 7, size(Intf_BS_t)); %log10(normrnd(0, 7)); %7dB std deviation, random normal, mean 0
PL_B1_intf = 22.7*log10(Intf_dist) + 27.0 + 20*log10(fc) + Intf_SF; %sf 3dB
PL_B1_2_intf = 40*log10(Intf_dist) + 7.56-17.3*log10(Intf_BS_t)-17.3*log10(Intf_MS_t)+2.7*log10(fc)+Intf_SF;
...
sum_intf_MP_mW = 0; 
%Worst Case attenuation model (BS Delay Spread to Self)
...
%We model interfering cells as noise, no delay spread
if(0)
    for i = 1:length(TDL_C_SCALED)
        if(Intf_pwr_dBm > PL_tot_intf + Intf_TDL_C_SCALED(i ,2)) %Ensure external direct inerference is lower bounded
            Intf_RX = Intf_pwr_dBm - (PL_tot_intf + Intf_TDL_C_SCALED(i ,2));
        else
            Intf_RX = 0;
        end
    end
